#!/usr/bin/env Rscript
# Generate figure S1 - Hyperparameter optimisation
source('src/config.R')
...
p1 <- p_train_activation + labs(tag = 'A') + size
p2 <- p_train_filters + labs(tag = 'B') + size
p3 <- p_train_kernel + labs(tag = 'C') + size
p4 <- p_train_layers + labs(tag = 'D') + size
p5 <- p_train_structure + labs(tag = 'E') + size
...
ggsave('figures/figures/figure_S1.pdf', figure, width = figure_width(figure), height = figure_height(figure), units = 'mm',
       device = cairo_pdf)
ggsave('figures/figures/figure_S1.png', figure, width = figure_width(figure), height = figure_height(figure), units = 'mm')
