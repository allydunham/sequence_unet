...
tool_speed <- tibble(tool = c("FoldX", "SIFT4G", "UNET (init)", "UNET (pred)"),
...
figure1 <- multi_panel_figure(width = c(52, 52, 36), height = c(65, 52/2, 52/2), panel_label_type = 'none', row_spacing = 0, column_spacing = 0) %>%
  fill_panel(p1, row = 1, column = 1:3) %>%
  fill_panel(p2, row = 2, column = 1) %>%
  fill_panel(p3, row = 3, column = 1) %>%
  fill_panel(p4, row = 2:3, column = 2) %>%
  fill_panel(p5, row = 2:3, column = 3)
