...
classifier_performance <- read_tsv("data/freq/all_model_testing.tsv") %>%
  mutate(threshold = c(`UNET`=0.5, `PreGraph UNET`=0.5, `Baseline CNN`=0.5, `ESM-1b Logits`=log(0.05),
                       `ESM-1b Top Model`=0.5, `BLOSUM62`=-2, `SIFT4G`=0.05)[model],
         pred_del = ifelse(model %in% c("SIFT4G", "BLOSUM62", "ESM-1b Logits"), pred < threshold, pred > threshold)) %>%
  ...
  mutate(model = factor(model, levels = c("BLOSUM62", "SIFT4G", "ESM-1b Logits", "ESM-1b Top Model", "Baseline CNN", "UNET", "PreGraph UNET")),
...
classifier_auc <- distinct(classifier_roc, model, model_auc, auc) %>%
  arrange(desc(auc)) %>%
  mutate(fpr = 1, tpr = c(0.33, 0.28, 0.23, 0.18, 0.13, 0.08, 0.03),
         col = TOOL_COLOURS[model])

thresholds <- tibble(model=c("UNET", "PreGraph UNET", "Baseline CNN", "ESM-1b Logits", "ESM-1b Top Model", "BLOSUM62", "SIFT4G"),
                     threshold=c(0.5, 0.5, 0.5, log(0.05), 0.5, -2, 0.05)) %>%
...
classifier_pr_auc <- distinct(classifier_roc, model, model_pr_auc, pr_auc) %>%
  arrange(desc(pr_auc)) %>%
  mutate(tpr = 1, precision = c(1, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7),
         col = TOOL_COLOURS[model])
...
p_legend <- get_legend(
  ggplot(leg, aes(x = model, y = roc_auc, fill = label)) +
    ...
    guides(fill = guide_legend(title = NULL, direction = "horizontal", nrow = 4, byrow = TRUE)) +
...
figure3 <- multi_panel_figure(width = c(40,65,65), height = c(50,40,65,20),
